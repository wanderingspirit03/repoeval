...
Git-history tasks generated by `repoeval generate --from-git-history` include both the target `source.commit` and, when available, `source.parent_commit`. During `repoeval run`, those tasks start their isolated repo from `source.parent_commit` so the runner is judged on recreating the target commit instead of seeing the already-completed final state. Generated fixtures put only paths that exist in the target commit in `expected_files`; deleted paths stay in `source.changed_files` so target-content verification can require the file to be absent. For changed files that already existed in the parent commit, RepoEval also compares the runner output with the target commit content; an unchanged parent copy does not pass just because the expected path exists. Added files continue to be scored by existence/diff plus configured verification commands. Git-history tasks without a parent commit, plus manual/YAML tasks, keep the default `HEAD` isolation behavior, and manual `expected_files` entries still mean the path must exist.
